...
import com.worldbank.app.activities.admin.AdminDashboardActivity;
...
public class AccountActivity extends AppCompatActivity {
    ...
    private LinearLayout llYourAccount, llPayment, llHistoryActivities, llPrivacySecurity, llAboutUs, llLogout, llViewAsAdmin;
    ...
    private void bindViews() {
        ...
        llViewAsAdmin = findViewById(R.id.llViewAsAdmin);
        ...
        db.collection("users").document(uid).get()
                .addOnSuccessListener(doc -> {
                    if (doc.exists()) {
                        ...
                        // FIX: Get the Boolean 'isAdmin' instead of the String 'role'
                        Boolean isAdmin = doc.getBoolean("isAdmin");
                        Log.d("AdminCheck", "isAdmin value: " + isAdmin);

                        tvUserName.setText(name != null ? name : sessionManager.getUserName());
                        tvUserLocation.setText(city != null ? city : "Lahore, Pakistan");

                        if (name != null) {
                            tvUserInitials.setText(getInitials(name));
                        }

                        // Toggle Admin View based on the Boolean value
                        if (llViewAsAdmin != null) {
                            llViewAsAdmin.setVisibility(Boolean.TRUE.equals(isAdmin) ? View.VISIBLE : View.GONE);
                        }
                    }
                })
    ...
    private void setupClickListeners() {
        ...
        if (llViewAsAdmin != null) {
            llViewAsAdmin.setOnClickListener(v ->
